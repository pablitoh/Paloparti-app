...
export default async function handler(
  ...
  try {
    ...
    // Verificar si el miembro existe en el grupo
    ...
    const existingAttendance = await prisma.$queryRaw<Array<{ id: string }>>`
      ...
      WHERE "userId" = ${user.id} AND "matchId" = ${matchId}
      LIMIT 1
    `;
    ...
    if (existingAttendance && existingAttendance.length > 0) {
      ...
      attendanceResult = await prisma.$executeRaw`
        INSERT INTO "MatchAttendance" (
          ...
          gen_random_uuid(), ${user.id}, ${matchId}, ${match.groupId}, ${match.date}, ${status}, NOW(), NOW()
        )
      `;
    }

    console.log(`Updated match attendance for user ${user.id} to ${status}`);
